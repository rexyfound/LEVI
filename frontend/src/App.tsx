...
function VoicePanel({ state }: { state: string }) {
  return (
    <div className="py-3">
      {[['STATUS', state],['VOICE','LEVI'],['STYLE','Calm, concise'],['OUTPUT','System default']].map(([label, value]) => (
        <NmRow key={label} label={label} value={value} valueColor={label === 'STATUS' && state !== 'IDLE' ? 'var(--violet-light)' : undefined} />
      ))}
    </div>
  );
}
...
export default function App() {
  ...
  const agentState = voiceState === 'SPEAKING'
    ? 'SPEAKING'
    : voiceState === 'PREPARING'
      ? 'PREPARING'
      : (coreState === "WAITING_CONFIRMATION" || coreState === "AWAITING_APPROVAL")
  ...
  const isProcessing = ["RECEIVING", "PLANNING", "EXECUTING", "VALIDATING", "WAITING_FOR_APPROVAL", "PREPARING", "SPEAKING"].includes(agentState);
  ...
  const stateLabel: Record<AgentState, string> = {
    ...
    VALIDATING: "Validating...", COMPLETED: "Completed.", FAILED: "Failed.", PREPARING: "Voice preparing...", SPEAKING: "Speaking...",
  };
      ...
      {openPanel && (
        <div className="absolute inset-0 z-30" style={{ pointerEvents: "none" }}>
          <div style={{ position: "absolute", left: 48, top: 0, bottom: 0, pointerEvents: "auto" }}>
            ...
            {openPanel === "voice" && <FloatingPanel title="VOICE" onClose={() => setOpenPanel(null)}><VoicePanel state={voiceState} /></FloatingPanel>}
